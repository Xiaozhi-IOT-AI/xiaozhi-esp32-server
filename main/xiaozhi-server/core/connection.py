...
class ConnectionHandler:
    ...
    def _initialize_intent(self):
        ...
        if hasattr(self, "loop") and self.loop:
            future = asyncio.run_coroutine_threadsafe(self.func_handler._initialize(), self.loop)
            if not hasattr(self, 'pending_futures'):
                self.pending_futures = []
            self.pending_futures.append(future)
    ...
    async def close(self, ws=None):
        ...
        try:
            ...
            if hasattr(self, "asr_audio_for_voiceprint"):
                self.asr_audio_for_voiceprint.clear() if hasattr(self.asr_audio_for_voiceprint, 'clear') else None
            ...
            if hasattr(self, "tts_text_buff"):
                self.tts_text_buff.clear() if hasattr(self.tts_text_buff, 'clear') else None
            ...
            if hasattr(self, 'asr_priority_thread') and self.asr_priority_thread and self.asr_priority_thread.is_alive():
                try:
                    # 在线程池中等待，避免阻塞主事件循环
                    await asyncio.get_event_loop().run_in_executor(
                        None,
                        lambda: self.asr_priority_thread.join(timeout=1)
                    )
            ...
            if hasattr(self, 'report_thread') and self.report_thread and self.report_thread.is_alive():
                try:
                    ...
                    await asyncio.get_event_loop().run_in_executor(
                        None,
                        lambda: self.report_thread.join(timeout=1)
                    )
                    if self.report_thread.is_alive():
                        self.logger.bind(tag=TAG).warning("报告线程未能在超时时间内退出")
                except Exception as e:
            ...
            if self.tts:
                try:
                    ...
                    self.logger.bind(tag=TAG).debug(f"关闭TTS资源时出错: {tts_error}")
                finally:
                    self.tts = None
            ...
            if hasattr(self, 'dialogue') and self.dialogue:
                try:
                    if hasattr(self.dialogue, 'dialogue'):
                        self.dialogue.dialogue.clear()
                    self.dialogue = None
                except Exception as dialogue_error:
                    self.logger.bind(tag=TAG).debug(f"清理dialogue时出错: {dialogue_error}")
            ...
            if hasattr(self, 'config'):
                self.config.clear() if isinstance(self.config, dict) else None
                self.config = None
            if hasattr(self, 'common_config'):
